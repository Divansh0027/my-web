    ...
    <div className="font-sans text-on-surface bg-surface pt-24 pb-20 min-h-screen">
      ...
      <div className="bg-surface-container border-b border-outline-variant/50 py-10 px-4 sm:px-6 lg:px-8 mb-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="text-gold-accent font-semibold text-xs tracking-wider uppercase">Direct-to-Owner Registry</div>
            <h1 className="text-3xl font-extrabold text-on-surface mt-1">Properties in Delhi NCR</h1>
          ...
          <button
            ...
            className="self-start md:self-auto px-5 py-2 rounded-lg bg-surface-container-high hover:bg-outline-variant text-on-surface-variant font-semibold text-xs transition-all border border-outline-variant/50"
          >
            Reset All Filters
          </button>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="hidden lg:block bg-surface-container border border-outline-variant/50 p-6 rounded-2xl h-fit space-y-6">
            ...
            <div className="space-y-3">
              <label htmlFor="search-query-ds" className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Search keyword</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-surface border border-outline-variant rounded-lg pl-9 pr-3 py-2 text-xs text-on-surface placeholder-slate-600 outline-none focus:border-gold-accent/50 focus:ring-1 focus:ring-gold-accent/50"
                />
              </div>
            </div>
            ...
            {/* Location checkboxes */}
            <div className="space-y-3">
              <p className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Locality</p>
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2 scrollbar-thin">
                {["Dwarka", "Gurugram", "Noida", "Greater Noida West", "Faridabad", "Rohini", "Pitampura", "Aerocity"].map((city) => {
                  ...
                  return (
                  <label htmlFor={uid} key={city} className="flex items-center gap-2.5 text-xs text-on-surface-variant cursor-pointer hover:text-on-surface">
                    <input id={uid}
                      type="checkbox"
                      checked={selectedLocations.includes(city)}
                      onChange={() => toggleLocation(city)}
                      className="rounded border-outline-variant text-gold-accent focus:ring-gold-accent/50 h-4 w-4"
                    />
                    {city}
                  </label>
                )})}
            ...
            <div className="space-y-3 border-t border-outline-variant/50 pt-4">
              <p className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Property Type</p>
              <div className="space-y-2">
                {["Flat", "Villa", "Plot", "Builder Floor", "Commercial"].map((t) => {
                  const uid = `filter-type-${t.replace(/\s+/g, '-')}`;
                  return (
                  <label htmlFor={uid} key={t} className="flex items-center gap-2.5 text-xs text-on-surface-variant cursor-pointer hover:text-on-surface">
                    <input id={uid}
                      type="checkbox"
            ...
            <div className="space-y-3 border-t border-outline-variant/50 pt-4">
              <div className="flex justify-between items-center text-xs">
                <label htmlFor="auto-listingsview-279" className="font-bold text-on-surface-variant uppercase tracking-wider">Max Budget</label>
              ...
              <input id="auto-listingsview-279"
                ...
                className="w-full accent-gold-accent bg-surface-container-high"
              />
            ...
            {/* BHK Toggles */}
            <div className="space-y-3 border-t border-outline-variant/50 pt-4">
              <p className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">BHK Type</p>
                  ...
                  return (
                    <button
                      ...
                      className={`py-1.5 rounded text-xs font-bold border transition-all cursor-pointer ${
                        isSelected 
                          ? "bg-gold-accent text-[#0F172A] border-gold-accent" 
            ...
            {/* Status Checkboxes */}
            <div className="space-y-3 border-t border-outline-variant/50 pt-4">
              <p className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Construction Status</p>
              <div className="space-y-2">
                {["Ready to Move", "Under Construction", "New Launch"].map((st) => {
                  const uid = `filter-status-${st.replace(/\s+/g, '-')}`;
                  return (
                  <label htmlFor={uid} key={st} className="flex items-center gap-2.5 text-xs text-on-surface-variant cursor-pointer hover:text-on-surface">
                    <input id={uid}
                      type="checkbox"
            ...
            {/* Posted By check */}
            <div className="space-y-3 border-t border-outline-variant/50 pt-4">
              <p className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Posted By</p>
              <div className="space-y-2">
                {["Owner", "Builder", "Agent"].map((p) => {
                  const uid = `filter-posted-${p.replace(/\s+/g, '-')}`;
                  return (
                  <label htmlFor={uid} key={p} className="flex items-center gap-2.5 text-xs text-on-surface-variant cursor-pointer hover:text-on-surface">
                    <input id={uid}
                      type="checkbox"
          ...
          <main className="lg:col-span-3 space-y-6">
            ...
            <div className="p-4 bg-surface-container rounded-xl border border-outline-variant/50 flex items-center justify-between flex-wrap gap-4 select-none">
              <div className="flex items-center gap-3 text-xs text-on-surface-variant font-medium">
                <span>Total Results: <b className="text-on-surface">{sortedListings.length}</b></span>
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="lg:hidden flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface-container-high text-on-surface-variant border border-outline-variant text-xs font-bold"
                >
                  <SlidersHorizontal className="h-3.5 w-3.5 text-gold-accent" />
                  Filters
                </button>
                ...
                <div className="hidden sm:flex items-center bg-surface-container-high rounded-lg p-0.5 border border-outline-variant">
                  <button
                    ...
                    className={`p-1.5 rounded-md ${viewMode === "grid" ? "bg-gold-accent text-[#0F172A]" : "text-on-surface-variant"}`}
                  >
                    <LayoutGrid className="h-3.5 w-3.5" />
                  </button>
                  <button
                    onClick={() => setViewMode("list")}
                    className={`p-1.5 rounded-md ${viewMode === "list" ? "bg-gold-accent text-[#0F172A]" : "text-on-surface-variant"}`}
                  >
            ...
            {isLoading || isLoadingData ? (
              ...
              <div className="bg-surface-container border border-outline-variant/50 text-center p-16 rounded-2xl max-w-lg mx-auto">
                ...
                <button
                  onClick={handleClearFilters}
                  className="px-6 py-2.5 rounded-xl bg-gold-accent text-[#0F172A] font-bold text-xs shadow-md"
                >
                  Reset Settings
                </button>
              </div>
                ...
                {paginatedListings.map((prop) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`bg-surface-container border border-outline-variant/50 rounded-2xl overflow-hidden shadow hover:border-gold-accent/30 transition-all group flex ${
                    ...
                    >
                      ...
                      <div className={`relative overflow-hidden ${viewMode === "list" ? "w-full md:w-2/5 shrink-0 h-56 md:h-full" : "h-60 w-full shrink-0"}`}>
                        ...
                        <div className="absolute top-3 left-3 flex gap-1">
                          {prop.featured && (
                            <span className="bg-gold-accent text-[#0F172A] font-bold text-[9px] uppercase px-2 py-0.5 rounded shadow">
                              Featured
                            </span>
                          )}
                          {prop.newLaunch && (
                            <span className="bg-emerald-500 text-on-surface font-bold text-[9px] uppercase px-2 py-0.5 rounded shadow">
                              New Launch
                            </span>
                          )}
                        </div>
                      ...
                      <div className="p-5 flex-1 flex flex-col justify-between">
                        <div>
                          <div className="flex justify-between items-center text-xs">
                            <span className="text-gold-accent font-bold text-xl">{formatPrice(prop.price)}</span>
                            <span className="text-on-surface-variant">{prop.type}</span>
                          </div>
                          ...
                          <div className="flex items-center gap-4 border-t border-outline-variant/50 pt-3 mt-3 text-xs text-on-surface-variant">
                            {prop.bhk && (
                              <div className="flex items-center gap-1">
                                <BedDouble className="h-3.5 w-3.5 text-outline" />
                                <span>{prop.bhk} BHK</span>
                              </div>
                            )}
                            <div className="flex items-center gap-1">
                              <Maximize className="h-3.5 w-3.5 text-outline" />
                              <span>{prop.area} {prop.areaUnit}</span>
                            </div>
                            {prop.availabilityStatus && (
                              <div className="flex items-center gap-1">
                                <span className="text-on-surface-variant bg-surface-container-high px-1.5 py-1 text-[10px] rounded uppercase font-semibold">{prop.availabilityStatus}</span>
                              </div>
                            )}
                          </div>
                        </div>
            ...
            {sortedListings.length > itemsPerPage && !isLoading && (
              <div className="flex items-center justify-between border-t border-outline-variant/50 pt-8 select-none text-xs">
                <button
                      ...
                      <button
                        ...
                        className={`h-8 w-8 rounded-lg font-bold transition-all cursor-pointer ${
                          currentPage === pageNo 
                            ...
                            : "bg-surface-container text-on-surface-variant hover:text-on-surface border border-outline-variant/50"
                        }`}
                      >
                        {pageNo}
                      </button>
        ...
        {isMobileFilterOpen && (
          <>
            ...
            >
              ...
              {/* Keyword Search */}
              <div className="space-y-3">
                <label htmlFor="search-query-mb" className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">Search keyword</label>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-outline" />
                  <input
                    id="search-query-mb"
                    ...
                    className="w-full bg-surface border border-outline-variant rounded-lg pl-9 pr-3 py-3 text-xs text-on-surface placeholder-slate-600 outline-none focus:border-gold-accent/50 focus:ring-1 focus:ring-gold-accent/50"
                  />
                </div>
              </div>
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold border ${
                          isSelected ? "bg-gold-accent/20 border-gold-accent text-on-surface" : "border-outline-variant/50 bg-surface-container text-on-surface-variant"
                        }`}
                      >
                        {city}
                      </button>
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center text-xs">
                  <label htmlFor="auto-listingsview-691" className="font-bold text-on-surface-variant uppercase tracking-wider">Max Budget</label>
                  <span className="text-gold-accent font-semibold">
                    {budgetMax >= 100000000 ? "Any Budget" : `₹${(budgetMax / 10000000).toFixed(1)} Cr`}
                  </span>
                </div>
              ...
              {/* Mobile BHK */}
              <div className="space-y-3">
                <p className="block text-xs font-bold text-on-surface-variant uppercase tracking-wider">BHK Preferences</p>
                    ...
                    return (
                      <button
                        ...
                        className={`py-2 rounded-lg text-xs font-bold border transition-all ${
                          isSelected ? "bg-gold-accent text-[#0F172A] border-gold-accent" : "border-outline-variant text-on-surface-variant"
                        }`}
                      >
                        {b} BHK
                      </button>
              ...
              <div className="flex gap-4 pt-4 border-t border-outline-variant/50">
                <button
                  onClick={handleClearFilters}
                  className="flex-1 py-3 border border-outline-variant rounded-xl text-center text-on-surface-variant text-xs font-bold"
                >
                  Clear All
                </button>
                <button
                  onClick={() => setIsMobileFilterOpen(false)}
                  className="flex-1 py-3 bg-gold-accent text-[#0F172A] text-xs font-bold rounded-xl text-center"
                >
                  Apply Filters
                </button>
              </div>
