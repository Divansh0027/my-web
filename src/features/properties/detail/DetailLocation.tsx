...
export function DetailLocation({ property }: DetailLocationProps) {
  const hasMapKey = !!import.meta.env.VITE_GOOGLE_MAPS_API_KEY
  ...
  return (
    <div className="p-8 bg-surface-container border border-outline-variant/50 rounded-2xl space-y-6">
      ...
      {/* Map Embed */}
      <div className="relative h-64 w-full bg-surface-container-low rounded-xl overflow-hidden border border-outline-variant/50">
        {hasMapKey ? (
          <iframe
          ...
          <div className="absolute inset-0 bg-surface/85 backdrop-blur-xs flex flex-col items-center justify-center p-6 text-center">
            ...
            <h3 className="text-on-surface font-bold text-sm tracking-tight">
              {property.location}
            </h3>
            <a
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {property.landmarks && property.landmarks.length > 0 ? (
            property.landmarks.map((land, i) => (
              <div
                key={i}
                className="flex justify-between items-center text-xs p-3.5 bg-surface-container-low rounded-xl border border-outline-variant/50"
              >
