      ...
      <div className="space-y-6 animate-fadeIn md:text-left text-xs text-on-surface-variant">
        <div>
          <h1 className="text-xl font-extrabold text-on-surface tracking-tight">
            Readiness Audit Compliance Diagnostics
          </h1>
          <p className="text-xs text-on-surface-variant">
            Validate real-time real estate guidelines, broker validation records, and digital
          ...
          <div className="space-y-4">
            ...
            <div className="bg-surface-container border border-outline-variant/50 rounded-2xl p-5 shadow-md space-y-4 flex flex-col justify-between">
                ...
                {isRunningDiagnostics ? (
                  <div className="py-6 flex flex-col items-center justify-center gap-3">
                    ...
                    <div className="text-center space-y-1">
                      <p className="text-on-surface text-[11px] font-black animate-pulse uppercase tracking-wider">
                        Scanning Internal Keys...
                      </p>
                      <p className="text-on-surface-variant text-[8px] font-mono">
                        Verifying broker RERA indices and database objects
                      </p>
                    </div>
                  </div>
                ) : auditPassed === true ? (
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setAuditPassed(null) // reset while running

                  // Compute real compliance checks
                  ...
                  const allPassed =
                    reraPass && propertiesPass && approvalsPass && enquiriesPass && consultantPass

                  setTimeout(() => {
                    setIsRunningDiagnostics(false)
                    setAuditPassed(allPassed)
                    if (allPassed) {
                      onShowNotification(
                        'Compliant audit finished! Digital credentials are in full order.',
                        'success',
                      )
                    } else {
                      onShowNotification(
                        'Audit finished with warnings. Please review the highlighted issues.',
                        'error',
                      )
                    }
                  }, 1800)
                ...
                className={`w-full py-3 rounded-xl font-bold font-sans text-xs flex items-center justify-center gap-2 cursor-pointer transition-all ${
                  isRunningDiagnostics
                    ...
                    : 'bg-gold-accent text-[var(--on-gold)] hover:bg-gold-hover hover:scale-105 shadow-md shadow active:scale-98'
                }`}
              >
                {isRunningDiagnostics ? (
                  <>
            ...
            <div className="p-4 bg-gold-accent/5 border border-gold-accent/10 rounded-2xl space-y-2">
              ...
              <p className="text-[9px] leading-relaxed text-on-surface-variant font-semibold">
                Every advertisement, circular, web portal, or social marketing post must display the
                registered corporate broker license clearly as defined under HRERA Rules, Section
                15. Real audits must verify documents on-site periodically.
              </p>
            </div>
          </div>
        </div>
