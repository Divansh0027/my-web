...
export default function UserManagement() {
  ...
  const { userSearch, setUserSearch, filteredUsers, handleToggleBanUser } = props
      ...
      <div className="space-y-6 animate-fadeIn">
        ...
        <div>
          <h1 className="text-xl font-extrabold text-on-surface tracking-tight">
            Simulated User Base ({filteredUsers.length} files)
          </h1>
          <p className="text-xs text-on-surface-variant">
            Suspend offending client accounts or override custom registration privileges instantly.
          </p>
        </div>

        <div className="bg-surface-container border border-outline-variant/50 rounded-2xl p-4 shadow-md relative">
          <Search className="absolute left-7 top-7 h-4 w-4 text-on-surface-variant" />
          <label htmlFor="search-accounts-input" className="sr-only">
            Search Accounts
          </label>
          <input
                  ...
                  filteredUsers.map((usr) => (
                    <tr key={usr.uid} className="hover:bg-surface/20 transition-all">
                      <td className="py-4 px-4 font-extrabold text-on-surface flex items-center gap-3">
                        ...
                        <div>
                          {usr.displayName}
                          <span className="block text-[8px] text-on-surface-variant font-mono">
                            UID: {usr.uid}
                          </span>
                        </div>
                      </td>
