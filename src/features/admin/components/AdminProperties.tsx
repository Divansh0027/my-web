...
import {
  ...
  useAdminCountsQuery,
} from '@/features/admin/hooks/useAdminPropertiesQuery'
...
export default function PropertyManagement() {
  ...
  const {
    ...
    propertySort === 'newest'
      ? 'createdAt'
      : propertySort === 'price-asc' || propertySort === 'price-desc'
        ? 'price'
        : 'createdAt',
      ...
      <div className="space-y-6 animate-fadeIn">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-extrabold text-on-surface tracking-tight">
              Real Estate Master Index ({totalFilteredCount} records)
            </h1>
            <p className="text-xs text-on-surface-variant">
              Search, filter, edit details manually, or process approvals in bulk.
            </p>
          </div>

          <button
            onClick={() => setIsAddModalOpen(true)}
            className="px-4 py-2.5 rounded-xl bg-gold-accent text-[var(--on-gold)] font-black text-xs flex items-center justify-center gap-1.5 cursor-pointer shadow active:scale-95 transition-all self-start sm:self-auto"
          >
            <Plus className="h-4 w-4 text-[var(--on-gold)] stroke-[3]" /> Add Manual Property
          </button>
        </div>
        ...
        <div className="bg-surface-container border border-outline-variant/50 rounded-2xl p-4 flex flex-col md:flex-row md:items-center gap-3.5 shadow-md">
          <div className="flex-1 relative">
            <Search className="absolute left-3.5 top-3 h-4 w-4 text-on-surface-variant" />
            <label htmlFor="search-listings-input" className="sr-only">
              Search Listings
            </label>
            <input
              ...
              className="w-full bg-surface border border-outline-variant/50 focus:border-gold-accent/40 rounded-xl pl-10 pr-4 py-2.5 text-xs text-on-surface placeholder-on-surface-variant outline-none transition-all"
            />
          </div>
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 rounded-lg bg-gold-accent hover:bg-gold-hover text-[var(--on-gold)] font-black text-[10px] cursor-pointer active:scale-95 transition-all"
              >
                Approve Batch
              </button>
              <button
              ...
              <tbody className="divide-y divide-white/5 text-xs text-on-surface-variant">
                {paginatedProperties.length === 0 ? (
                  ...
                  paginatedProperties.map((prop) => (
                    <tr
                      key={prop.id}
                      className={`transition-all text-xs font-sans ${selectedProperties.includes(prop.id) ? 'bg-gold-accent/5' : 'hover:bg-surface/20'}`}
                    >
                      ...
                      <td className="py-4 px-4 text-center">
                        <span
                          className={`inline-block px-2.5 py-1 rounded-full text-[9px] font-extrabold border leading-none uppercase ${
                        ...
                        {prop.featured && (
                          <span className="block mt-1 text-[8px] text-gold-accent font-bold uppercase">
                            Featured
                          </span>
                        )}
                      </td>
                      <td className="py-4 px-4">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() =>
                              handlePropertyApprovalToggle(prop.id, prop.moderationStatus)
                            }
                            className={`p-1.5 rounded-lg border flex items-center justify-center transition-all cursor-pointer ${
                              prop.moderationStatus === 'live'
                                ...
                                : 'bg-gold-accent hover:bg-gold-hover text-[var(--on-gold)] border-transparent'
                            }`}
