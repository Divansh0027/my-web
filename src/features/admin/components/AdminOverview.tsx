...
import {
  ...
  useAdminCountsQuery,
  useAdminPendingPropertiesQuery,
          ...
          {[
            {
              ...
              desc: 'Publicly visible to users',
            },
            {
              title: 'Pending Approvals',
              value: pendingApprovalsCount,
              desc: 'Waiting for manual review',
              ...
              {pendingProperties.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-center gap-2">
                  ...
                  <p className="text-xs text-on-surface-variant font-semibold">
                    All property records verified!
                  </p>
                  <p className="text-[10px] text-on-surface-variant">
                    Every direct submit has been physically audited.
                  </p>
                </div>
              ) : (
                pendingProperties.map((prop) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-gold-accent font-black mt-0.5">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        onClick={() =>
                          props.handlePropertyApprovalToggle(prop.id, prop.moderationStatus)
                        }
                        className="px-3 py-1.5 rounded-lg bg-gold-accent hover:bg-gold-hover text-[var(--on-gold)] font-black text-[10px] flex items-center gap-1 cursor-pointer transition-all active:scale-95"
            ...
            <div className="space-y-3 max-h-[340px] overflow-y-auto pr-1 scrollbar-thin">
              {enquiries.slice(0, 5).map((enq, idx) => (
                ...
                >
                  ...
                  <p className="text-[10px] text-gold-accent font-bold truncate">
                    Prop: {enq.propertyName}
                  </p>
                  <p className="text-[10px] text-on-surface-variant italic line-clamp-2">
                    "{enq.message}"
                  </p>
                </div>
              ))}
