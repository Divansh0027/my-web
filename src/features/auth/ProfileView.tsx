...
import { updateUserProfileDetails, logoutUser, getUserEnquiries } from '@/firebase'
import { Enquiry, Property } from '@/shared/types/types'
...
export default function ProfileView({
  ...
  const [activeTab, setActiveTab] = useState<'listings' | 'favorites' | 'enquiries' | 'settings'>(
    'listings',
  )
  ...
  useEffect(() => {
    if (currentUser?.uid) {
      getUserEnquiries(currentUser.uid).then(setEnquiries)
    }
  }, [currentUser?.uid])
  ...
  const handleClearEnquiries = () => {
    // Deleting from Firestore is not implemented for users yet
    // Just clearing local state for now
    setEnquiries([])
    onShowNotification('Activity log hidden from view.', 'info')
  }
        ...
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-on-surface-variant text-xs mt-2 font-medium leading-relaxed">
              Organize your listings, track saved bookmark shortlists, manage scheduled site tours,
              and adjust contact records.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 space-y-8">
                ...
                <div className="space-y-1.5 text-center sm:text-left">
                  ...
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 text-xs text-on-surface-variant">
                    <span className="flex items-center gap-1.5">
                      ...
                      {user?.email || 'guest@shivsayaproperties.com'}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Phone className="h-3.5 w-3.5 text-on-surface-variant" />{' '}
                      {editPhone || 'Add contact number'}
                    </span>
                  </div>
                </div>
              ) : (
                <form onSubmit={handleSaveProfile} className="w-full space-y-3.5 pt-1">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                    <div className="space-y-1">
                      ...
                      <input
                        id="auto-profileview-156"
                        type="text"
                        required
                        value={editName}
                        onChange={(e) => setEditName(e.target.value)}
                        className="w-full bg-surface border border-outline-variant rounded-lg px-3 py-2 text-xs text-on-surface placeholder-on-surface-variant outline-none focus:border-gold-accent/40"
                      />
                    </div>
                    <div className="space-y-1">
                      <label
                        htmlFor="auto-profileview-166"
                        className="text-[10px] font-bold uppercase text-on-surface-variant"
                      >
                      ...
                      <input
                        id="auto-profileview-166"
                        type="email"
                        disabled
                        value={editEmail}
                        className="w-full bg-surface/60 border border-outline-variant/50 rounded-lg px-3 py-2 text-xs text-on-surface-variant outline-none"
                      />
                    </div>
                    <div className="space-y-1">
                      <label
                        htmlFor="auto-profileview-175"
                        className="text-[10px] font-bold uppercase text-on-surface-variant"
                      >
                        Contact (+91 Indian)
                      </label>
                  ...
                  <div className="flex gap-2 justify-end pt-1">
                    ...
                    <button
                      type="submit"
                      disabled={isSaving}
                      className="px-4 py-1.5 bg-gold-accent hover:bg-gold-hover hover:scale-105 shadow-md active:scale-98 transition-all text-[var(--on-gold)] rounded-lg text-xs font-bold flex items-center gap-1.5 cursor-pointer"
                    >
            ...
            {userProperties.length === 0 ? (
              <div className="text-center py-10 space-y-3">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-gold-accent text-[var(--on-gold)] text-xs font-bold rounded-lg hover:bg-gold-hover hover:scale-105 shadow-md active:scale-98 cursor-pointer"
                >
              ...
              <div className="space-y-4">
                {userProperties.map((prop) => {
                  ...
                  const handleDeleteClick = async (e: React.MouseEvent) => {
                    ...
                    if (
                      window.confirm(
                        `Are you sure you want to permanently remove "${prop.title}" listing? This action is irreversible.`,
                      )
                    ) {
                      if (onDeleteProperty) {
                        try {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        <p className="text-[10px] text-gold-accent font-black">
                          {(typeof prop.price === 'number'
                            ? formatPrice(prop.price)
                            : String(prop.price)) || `₹${(prop.price / 100000).toFixed(0)} Lakhs`}
                        </p>
                      </button>
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <p className="text-gold-accent font-bold text-xs">
                          {(typeof prop.price === 'number'
                            ? formatPrice(prop.price)
                            : String(prop.price)) || `₹${(prop.price / 100000).toFixed(0)} Lakhs`}
                        </p>
                    ...
                    <div className="pt-3 border-t border-outline-variant/50 mt-3 flex justify-between items-center">
                      <span className="text-[9px] font-bold text-on-surface-variant bg-surface-container-high px-2 py-0.5 rounded uppercase">
                        {prop.type}
                      </span>
                      <button
                        onClick={() => navigate(`/property/${prop.id}`)}
        ...
        {activeTab === 'enquiries' && (
          <div className="bg-surface-container border border-outline-variant/50 rounded-3xl p-6 sm:p-8 space-y-6">
            ...
            {enquiries.length === 0 ? (
              <p className="text-xs text-on-surface-variant leading-relaxed py-6 text-center italic">
                No callbacks or site tours requested in this browser session. Browse properties to
                schedule!
              </p>
            ) : (
              <div className="space-y-4">
                  ...
                  >
                    <div className="flex justify-between items-center flex-wrap gap-2 text-xs">
                      ...
                      <span className="text-on-surface-variant text-[10px]">
                        {enq.dateStr ? new Date(enq.dateStr).toLocaleDateString() : ''}
                      </span>
                    </div>
          ...
          <div className="bg-surface-container border border-outline-variant/50 rounded-3xl p-6 sm:p-8 space-y-8">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (
                        window.confirm(
                          'Are you sure you want to completely sign out and flush all local caches?',
                        )
                      ) {
                        await logoutUser()
                        localStorage.clear()
            ...
            <div className="space-y-3">
              ...
              <div className="p-4 bg-surface border border-outline-variant/50 rounded-2xl flex items-center justify-between gap-3">
                ...
                <label
                  htmlFor="auto-profileview-push"
                  className="relative inline-flex items-center cursor-pointer"
                  aria-label="Toggle push notifications"
                >
                  <input
                    id="auto-profileview-push"
                    type="checkbox"
                    ...
                    onChange={async (e) => {
                      if (e.target.checked) {
                        ...
                        if (token) {
                          setPushEnabled(true)
                          onShowNotification('Push notifications enabled.', 'success')
                        } else {
                          e.target.checked = false
                          onShowNotification('Permission denied or not supported.', 'error')
                        }
                      } else {
                        // Cannot strictly un-request permission via JS, but we can update state
                        setPushEnabled(false)
                        onShowNotification(
                          'Push notifications muted (Device settings may still apply).',
                          'info',
                        )
                      }
                    }}
                ...
                >
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      onShowNotification(
                        `Effects animations ${enabled ? 'enabled' : 'muted'}.`,
                        'success',
                      )
                    }}
                  />
