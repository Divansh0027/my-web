...
export default function LoginModal({ isOpen, onClose, initialTab = 'login' }: LoginModalProps) {
  ...
  const validatePhone = (phone: string) => {
    // 10 digit check and Indian prefix
  ...
  const isLoginDisabled =
    !loginEmail || !loginPassword || !!errors.loginEmail || !!errors.loginPassword || isSubmitting
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      trackUserEvent('login', { method: 'google' })
      if (result.success) {
        onClose()
      } else {
      ...
      if (
        ...
        friendlyMessage =
          "Google Sign-In is not enabled in your Firebase project yet. Please go to your Firebase Console > Authentication > Sign-in method, click 'Add new provider', choose 'Google', and enable it."
      }
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      trackUserEvent('login', { method: 'email' })
      if (result.success) {
        onClose()
      } else {
        setAuthError(result.error)
      ...
      if (
        ...
        friendlyMessage.includes('auth/wrong-password') ||
        friendlyMessage.includes('wrong-password')
      ) {
        ...
        friendlyMessage =
          "Email/Password sign-in is not enabled in your Firebase console. Please go to your Firebase Console > Authentication > Sign-in method tab, and enable the 'Email/Password' provider."
      }
      setAuthError(friendlyMessage)
    } finally {
      setIsSubmitting(false)
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        friendlyMessage =
          "Email/Password sign up is not enabled in your Firebase console. Please go to your Firebase Console > Authentication > Sign-in method tab, click 'Add new provider', choose 'Email/Password', and enable it."
      }
      setAuthError(friendlyMessage)
    } finally {
      setIsSubmitting(false)
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div
          id="login-modal-root"
          className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 font-sans select-none"
        >
            ...
            >
              ...
              <div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="mt-4 pt-4 border-t border-outline-variant/30 text-[10px] text-center text-on-surface-variant">
                    ...
                    <div className="flex bg-surface/80 p-1 rounded-xl border border-outline-variant/50">
                      <button
                        ...
                        className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                          activeTab === 'login'
                            ? 'bg-gold-accent text-[var(--on-gold)] shadow'
                    ...
                    activeTab === 'login' ? (
                      ...
                      <form onSubmit={handleLoginSubmit} className="space-y-4">
                        ...
                        <div className="space-y-1.5 animate-fadeIn">
                          <label htmlFor="login-email-input" className="sr-only">
                            Email Address
                          </label>
                          <div className="relative">
                            ...
                            <input
                              ...
                              className={`w-full bg-surface border rounded-xl pl-10 pr-4 py-3 text-xs text-on-surface placeholder-on-surface-variant outline-none focus:border-gold-accent/50 ${
                                errors.loginEmail
                                  ? 'border-red-500/50'
                        ...
                        <div className="space-y-1.5">
                          <div className="flex items-center justify-between">
                            <label htmlFor="login-password-input" className="sr-only">
                              Password
                            </label>
                            <button
                              type="button"
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full bg-surface border border-outline-variant/50 rounded-xl pl-10 pr-10 py-3 text-xs text-on-surface placeholder-on-surface-variant outline-none focus:border-gold-accent/50"
                            />
                            <button
                              type="button"
                              onClick={() => setShowLoginPassword(!showLoginPassword)}
                              aria-label="Toggle password visibility"
                              className="absolute right-3.5 top-3.5 h-5 w-5 text-on-surface-variant hover:text-on-surface-variant flex items-center justify-center cursor-pointer"
                            >
                              {showLoginPassword ? (
                      ...
                      <form onSubmit={handleSignupSubmit} className="space-y-4">
                        {/* Name */}
                        <div className="space-y-1.5 animate-fadeIn">
                          <label htmlFor="signup-name-input" className="sr-only">
                            Full Name
                          </label>
                          <div className="relative">
                            <User className="absolute left-3.5 top-3.5 h-4 w-4 text-on-surface-variant" />
                            <input
                        ...
                        {/* Email */}
                        <div className="space-y-1.5">
                          <label htmlFor="signup-email-input" className="sr-only">
                            Email Address
                          </label>
                          <div className="relative">
                            <Mail className="absolute left-3.5 top-3.5 h-4 w-4 text-on-surface-variant" />
                        ...
                        {/* Phone */}
                        <div className="space-y-1.5">
                          <label htmlFor="signup-phone-input" className="sr-only">
                            Verified Contact Number
                          </label>
                          <div className="relative">
                            <Phone className="absolute left-3.5 top-3.5 h-4 w-4 text-on-surface-variant" />
                            <span className="absolute left-9.5 top-3.5 text-xs text-on-surface-variant select-none font-bold">
                              +91
                            </span>
                            <input
                              ...
                              className={`w-full bg-surface border rounded-xl pl-19 pr-4 py-3 text-xs text-on-surface placeholder-on-surface-variant outline-none focus:border-gold-accent/50 ${
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            <label htmlFor="signup-password-input" className="sr-only">
                              Password
                            </label>
                            <div className="relative">
                              <Lock className="absolute left-3.5 top-3.5 h-4 w-4 text-on-surface-variant" />
                              <input
                                ...
                                className={`w-full bg-surface border rounded-xl pl-10 pr-10 py-3 text-xs text-on-surface placeholder-on-surface-variant outline-none focus:border-gold-accent/50 ${
                          ...
                          <div className="space-y-1.5">
                            <label htmlFor="signup-confirm-input" className="sr-only">
                              Confirm
                            </label>
                            <div className="relative">
                              <Lock className="absolute left-3.5 top-3.5 h-4 w-4 text-on-surface-variant" />
                        ...
                        {/* Agree T&C checkbox */}
                        <label
                          htmlFor="signup-agree-checkbox"
                          className="flex items-start gap-2.5 text-[11px] text-on-surface-variant hover:text-on-surface-variant cursor-pointer pt-1 select-none"
                        >
                          <input
                      ...
                      {forgotSuccess ? (
                        <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-emerald-400 text-xs space-y-3">
                          ...
                          <p className="leading-relaxed">
                            Password reset link sent to your registered email! Please click on the
                            link inside the email to configure a new password.
                          </p>
                          <button
                            type="button"
                            onClick={() => {
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-1.5">
                            <label htmlFor="forgot-email-input" className="sr-only">
                              Email Address
                            </label>
                            <div className="relative">
                              <Mail className="absolute left-3.5 top-3.5 h-4 w-4 text-on-surface-variant" />
