...
export default React.memo(function Navbar({
  ...
  const navLinks = [
    ...
    { name: t('nav.properties'), view: 'properties' },
    { name: t('nav.services'), view: 'services_sec' },
    { name: t('nav.about'), view: 'about_sec' },
    { name: t('nav.contact'), view: 'contact_sec' },
  ]
  ...
  return (
    <nav
      ...
      id="main-navbar"
      className={`sticky top-0 z-50 w-full transition-all duration-300 font-sans bg-surface/90 backdrop-blur-xl border-b ${
        isScrolled ? 'border-outline-variant/50 shadow-sm' : 'border-transparent'
      }`}
    >
      <div className="mx-auto w-full max-w-[1280px] px-4 md:px-6 h-[64px] lg:h-[72px] flex items-center justify-between relative">
        {/* Logo */}
        <div
          role="button"
          tabIndex={0}
        ...
        >
          ...
          <div className="text-base sm:text-lg font-bold tracking-tight text-on-surface whitespace-nowrap">
            Shiv{' '}
            <span className="text-gold-accent relative font-semibold">
              Saya<span className="absolute bottom-0 left-0 w-full h-[2px] bg-gold-accent"></span>
            </span>
          </div>
        </div>
          ...
          {user ? (
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="flex items-center gap-2 bg-surface-container-high/40 border border-outline-variant/50 pl-3 pr-2 py-1 rounded-full hover:bg-surface-container-high transition-colors cursor-pointer select-none"
              >
                {user.photoURL ? (
                  <img
                    width={24}
                ...
                <span className="text-xs font-semibold text-on-surface max-w-[80px] truncate">
                  {user.displayName}
                </span>
                <ChevronDown
                  className={`h-3.5 w-3.5 text-on-surface-variant transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`}
                />
              </button>
            ...
            <button
              onClick={handleLogin}
              className="flex items-center gap-2 px-4 py-1.5 text-xs font-semibold text-on-surface-variant hover:text-on-surface bg-surface-container-high/50 hover:bg-surface-container-high border border-outline-variant rounded-full transition-all cursor-pointer"
            >
              <UserIcon className="h-4 w-4 text-gold-accent" />
              Login / Signup
            </button>
                  ...
                  {user ? (
                    ...
                    <div className="text-center py-1">
                      <p className="text-xs text-on-surface-variant mb-3.5 leading-relaxed">
                        Access your direct-owner panel, save listings, and post properties.
                      </p>
                      <button
                        onClick={handleLogin}
                        className="w-full py-2.5 flex items-center justify-center gap-2 bg-gold-accent text-xs font-black text-[var(--on-gold)] rounded-lg transition-all cursor-pointer"
                      >
